...
// Malvern areas
const MALVERN_AREAS = [
  { id: 'malvern', name: 'Malvern', center: [-2.3258, 52.1120] as [number, number], zoom: 13, description: 'Malvern Hills, UK' },
]
...
export default function MalvernPage() {
    ...
    const initMap = async () => {
      try {
        ...
        map.current = new mapboxgl.Map({
          ...
          center: MALVERN_AREAS[0].center,
          zoom: MALVERN_AREAS[0].zoom
        })
              ...
              new mapboxgl.Popup({ offset: 25 })
                .setHTML(`
                  <div style="padding: 8px; min-width: 150px;">
                    ...
                    <p style="font-size: 12px; color: #666; margin-bottom: 8px;">${config.label}</p>
                    ${venue.address ? `<p style="font-size: 11px; color: #888;">${venue.address}</p>` : ''}
                  </div>
                `)
            )
            .addTo(map.current)
  ...
  const loadVenues = async () => {
    ...
    try {
      ...
      // Filter for Malvern hub
      const malvernVenues = data.filter(v => v.hub === 'Malvern' || v.hub === 'malvern')
      setVenues(malvernVenues)
    } catch (err) {
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6 border-b bg-gradient-to-r from-green-500/10 to-emerald-500/10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">Malvern Hub</h1>
              <p className="text-muted-foreground">Malvern Hills, UK - Venues & Accommodation</p>
            </div>
          </div>
        </div>
          ...
          <Card className="h-[500px] relative overflow-hidden">
            {loading ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-muted-foreground">No venues found for Malvern hub</p>
                <p className="text-sm text-muted-foreground">Add venues with hub="Malvern" to see them here</p>
              </div>
            ) : (
        ...
        <div className="w-80 border-l bg-muted/30 flex flex-col">
          <div className="p-4 border-b bg-green-500/5">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setSelectedVenue(null)}>×</Button>
            </div>
          ...
          <div className="flex-1 overflow-auto p-4 space-y-4">
            {selectedVenue.description && <p className="text-sm text-muted-foreground">{selectedVenue.description}</p>}
            {selectedVenue.address && <p className="text-sm">{selectedVenue.address}</p>}
          </div>
        </div>
      )}
    </div>
