...
const MORZINE_AREAS = [
  { id: 'morzine', name: 'Morzine', center: [6.7070, 46.1792] as [number, number], zoom: 13, description: 'French Alps' },
]

export default function MorzinePage() {
    ...
    const initMap = async () => {
      try {
        ...
        map.current = new mapboxgl.Map({
          ...
          center: MORZINE_AREAS[0].center,
          zoom: MORZINE_AREAS[0].zoom
        })
  ...
  const loadVenues = async () => {
    ...
    try {
      ...
      const morzineVenues = data.filter(v => v.hub === 'Morzine')
      setVenues(morzineVenues)
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      <div className="flex-1 overflow-auto">
        <div className="p-6 border-b bg-gradient-to-r from-cyan-500/10 to-sky-500/10">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-cyan-500/20 rounded-xl flex items-center justify-center">
              <Snowflake className="h-6 w-6 text-cyan-600" />
            </div>
            <div>
              <h1 className="text-2xl font-bold">Morzine Hub</h1>
              <p className="text-muted-foreground">French Alps - Ski resort town</p>
            </div>
          </div>
        </div>
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-4 gap-4">
            <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Total Venues</p><p className="text-2xl font-bold">{venues.length}</p></div><MapPin className="h-6 w-6 text-cyan-500" /></div></CardContent></Card>
          ...
          <Card className="h-[500px] relative overflow-hidden">
            ...
            ) : venues.length === 0 ? (<div className="absolute inset-0 flex flex-col items-center justify-center gap-4"><Snowflake className="h-8 w-8 text-muted-foreground" /><p className="text-muted-foreground">No venues found for Morzine hub</p><p className="text-sm text-muted-foreground">Add venues with hub="Morzine" to see them here</p></div>
