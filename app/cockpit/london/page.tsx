...
const LONDON_AREAS = [
  { id: 'london', name: 'SW London', center: [-0.1687, 51.4567] as [number, number], zoom: 13, description: 'Wandsworth, Clapham, Balham' },
]
...
export default function LondonPage() {
    ...
    const initMap = async () => {
      try {
        ...
        map.current = new mapboxgl.Map({
          ...
          center: LONDON_AREAS[0].center,
          zoom: LONDON_AREAS[0].zoom
        })
  ...
  const loadVenues = async () => {
    ...
    try {
      ...
      const londonVenues = data.filter(v => v.hub === 'London')
      setVenues(londonVenues)
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      <div className="flex-1 overflow-auto">
        <div className="p-6 border-b bg-gradient-to-r from-blue-500/10 to-indigo-500/10">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-blue-500/20 rounded-xl flex items-center justify-center">
              <Building2 className="h-6 w-6 text-blue-600" />
            </div>
            <div>
              <h1 className="text-2xl font-bold">London Hub</h1>
              <p className="text-muted-foreground">SW London - Wandsworth, Clapham, Balham</p>
            </div>
          </div>
        </div>
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-4 gap-4">
            <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Total Venues</p><p className="text-2xl font-bold">{venues.length}</p></div><MapPin className="h-6 w-6 text-blue-500" /></div></CardContent></Card>
          ...
          <Card className="h-[500px] relative overflow-hidden">
            ...
            ) : venues.length === 0 ? (<div className="absolute inset-0 flex flex-col items-center justify-center gap-4"><MapPin className="h-8 w-8 text-muted-foreground" /><p className="text-muted-foreground">No venues found for London hub</p><p className="text-sm text-muted-foreground">Add venues with hub="London" to see them here</p></div>
